...
savedir=mso-3dairq-fenics-result-`date +"%Y-%m-%d-%T"`
...
mkdir results
mpirun -n $NO_CORES singularity exec -B \$PWD/unicorn:/home -B /mnt  -B /scratch --pwd /home/3DAirQualityPredictionPilot $IMAGE_NAME ./3dAirQualityPredictionPilot --config parameters.cfg > log1 2>log2
...
ssh airq@193.224.129.162 'mkdir /nfs/results/$savedir'
scp -r results/ airq@193.224.129.162:/nfs/results/$savedir
